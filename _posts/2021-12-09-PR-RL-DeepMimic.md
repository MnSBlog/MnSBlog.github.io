...
\phi\in[0,\ 1]
$$
로 표현됩니다.사실 애니메이션 변수로 봐도 되는데, 모션이 시작되면 0이고 모션이 종료되면 1이 됩니다. 그래서 매 모션의 사이클마다 0으로 초기화 됩니다. 정책들은 추가적인 업무들을 달성하기 위해 훈련됩니다. 타겟을  타격하거나 특정 방향으로 걸어가는 등 업에 맞춘 목표 
$$
g
$$
가 제공됩니다. 그리고 관절마다 `PD controller`를 통한 운동 방향을 정하는 것을 액션으로 정의합니다. 그리고 액션의 갱신은 33ms마다 이뤄집니다. 또한 관절은 동그란 형태로 전재하니 축에 임의의 각도로서 표현합니다. 
...
\pi
$$
는 인공신경망 모델이며 a given s and g 조건부 분포 
$$
\pi(a|s,\ g)
$$
로 표현됩니다. 확률 분포는 정규분포를 가정하에 모델링이 되며 에이전트에 적용되는 state의 평균 
$$
\mu(s)
$$
와 고정된 대각 공분산 행렬 
...
\Sigma
$$
을 씁니다. 이를 아래로서 정리합니다. 밑의 내용이 더 친숙할 수 있습니다.
...
입력은 1024개의 노드, 512개의 노드로 구성된 두 개의 전연결 레이어입니다. `ReLU`를 활성함수로 쓰는 아주 일반적인 구조이고 또 높이를 가진 맵을 행렬형태로 입력되어 CNN을 거친 다음 1024개의 첫 전연결 레이어에 합성됩니다. 구조는 아래와 같습니다.
...
r_t
$$
는 각 스텝 t마다 두 가지의 주제를 가진채로 보상이 이뤄집니다. 참조 모션과 얼마나 일치하는지, 추가적인 업무 목표에 달성하는지 봅니다.
...
r_t^G
$$
는 업무 목표 달성에 대한 보상입니다. 
$$
r_t^I
$$
는 참조모션에 대한 일치 보상입니다.

두 보상함수는 모두 좀 더 상세한 설명을 할 수 있습니다. 아래로서요.
...
r_t^p
$$
는 포즈 보상입니다. 각 스텝별 참조 모션으로부터 관절이 얼마나 맞아떨어졌는지, 틀리다면 얼마나 틀렸는지 모션 시퀀스 
$$
\hat{q_t}
$$
를 이용하여 비교합니다. 이는 아래처럼 표현합니다.


$$
r_t^p=\exp\left[-2\left(\sum_j||\hat{q_t}^j\ominus q_t^j||^2\right)\right]
$$
...
v_t^v
$$
는 이름 그대로 목표 속도와의 편차를 설명합니다. 


$$
r_t^v=\exp\left[-0.1\left(\sum_j||\hat{\dot{q_t}}^j-\dot{q}_t^j||^2\right)\right]
$$


로봇 끝 단에 있는 end-effector 보상은 
$$
r_t^e
$$
라고 합니다. 이는 모델의 손과 발이 참조모션과 얼마나 유사한지를 따집니다. end-effector는 양 발과 양 손으로 총 4개입니다.


$$
r_t^e=\exp\left[-40\left(\sum_e||\hat{p_t}^e-p_t^e||^2\right)\right]
$$
